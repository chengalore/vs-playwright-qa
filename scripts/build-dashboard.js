...
function generateDashboard(history, compareImages, singleUrlHistory, metrics) {
  // ═══ COMPUTE HEALTH SUITE METRICS FROM REAL DATA ═══
  const latestRun = history[0] || {};
  const prevRun = history[1] || {};
  const last7Runs = history.slice(0, 7);

  // Pass rate
  const passRate = latestRun.summary ? ((latestRun.summary.passed / latestRun.summary.total) * 100).toFixed(1) : '0.0';
  const passRatePrev = prevRun.summary ? ((prevRun.summary.passed / prevRun.summary.total) * 100).toFixed(1) : passRate;
  const passRateDelta = (passRate - passRatePrev).toFixed(1);

  // Missing stores
  const missingCount = latestRun.summary?.widgetMissing || 0;
  const missingCountPrev = prevRun.summary?.widgetMissing || 0;
  const missingDelta = missingCount - missingCountPrev;

  // Store counts
  const totalStores = latestRun.summary?.total || 0;
  const healthyStores = latestRun.summary?.passed || 0;
  const botBlocked = (latestRun.botProtected || []).length;
  const botBlockRate = totalStores > 0 ? ((botBlocked / totalStores) * 100).toFixed(1) : '0.0';

  // Flake rate (stores that flipped pass<->miss in last 7 runs)
  let totalFlips = 0;
  for (let i = 0; i < last7Runs.length - 1; i++) {
    const curr = last7Runs[i];
    const prev = last7Runs[i + 1];
    if (!curr?.summary || !prev?.summary) continue;

    const currMissing = new Set((curr.widgetMissingStores || []).map(s => s.store));
    const prevMissing = new Set((prev.widgetMissingStores || []).map(s => s.store));

    // Count flips
    currMissing.forEach(store => { if (!prevMissing.has(store)) totalFlips++; });
    prevMissing.forEach(store => { if (!currMissing.has(store)) totalFlips++; });
  }
  const flakeRate = totalStores > 0 ? ((totalFlips / (totalStores * 6)) * 100).toFixed(1) : '0.0';

  // Health score formula: (passRate * 0.6) + (failed===0 ? 20 : 0) + (missing<=3 ? 20 : missing<=6 ? 10 : 0)
  const healthScore = Math.round(
    (parseFloat(passRate) * 0.6) +
    ((latestRun.summary?.failed || 0) === 0 ? 20 : 0) +
    (missingCount <= 3 ? 20 : missingCount <= 6 ? 10 : 0)
  );

  // Delta formatting helpers
  const deltaStyle = (val) => val > 0 ? '#f85149' : val < 0 ? '#3fb950' : '#8b949e';
  const deltaArrow = (val) => val > 0 ? '▲' : val < 0 ? '▼' : '—';
...
<header id="topbar">
  ...
  <nav class="topnav">
    ...
    <button onclick="showPanel('health-suite')" id="btn-health-suite">📊 Health Suite</button>
  </nav>
...
<main id="content">
  ...
  <!-- Health Suite -->
  <div class="panel" id="panel-health-suite">
    <h1>Health Monitoring Suite</h1>
    <p class="panel-subtitle">Operational intelligence · Last updated \${new Date().toLocaleString('en-GB', {day:'2-digit',month:'short',year:'numeric',hour:'2-digit',minute:'2-digit',timeZoneName:'short'})}</p>

    <div style="display:flex;border-bottom:2px solid #21262d;margin-bottom:18px">
      <button onclick="switchHealthTab('qa',this)" class="health-tab active" style="padding:9px 24px;font-size:13px;font-weight:500;cursor:pointer;border:none;background:none;color:#8b949e;border-bottom:2px solid transparent;margin-bottom:-2px;transition:all .15s">QA dashboard</button>
      <button onclick="switchHealthTab('client',this)" class="health-tab" style="padding:9px 24px;font-size:13px;font-weight:500;cursor:pointer;border:none;background:none;color:#8b949e;border-bottom:2px solid transparent;margin-bottom:-2px;transition:all .15s">Client dashboard</button>
      <button onclick="switchHealthTab('sales',this)" class="health-tab" style="padding:9px 24px;font-size:13px;font-weight:500;cursor:pointer;border:none;background:none;color:#8b949e;border-bottom:2px solid transparent;margin-bottom:-2px;transition:all .15s">Sales dashboard</button>
    </div>

    <div id="health-qa-panel" class="health-panel">
      <div style="display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:8px;margin-bottom:14px">
        <div style="background:#161b22;border:1px solid #21262d;border-radius:8px;padding:12px 14px">
          <div style="font-size:10px;color:#8b949e;text-transform:uppercase;letter-spacing:.04em;margin-bottom:5px">Health score</div>
          <div style="font-size:28px;font-weight:600;line-height:1;color:#3fb950">${healthScore}%</div>
          <div style="font-size:10px;color:#8b949e;margin-top:4px">target ≥ 90%</div>
        </div>
        <div style="background:#161b22;border:1px solid #21262d;border-radius:8px;padding:12px 14px">
          <div style="font-size:10px;color:#8b949e;text-transform:uppercase;letter-spacing:.04em;margin-bottom:5px">Pass rate</div>
          <div style="font-size:28px;font-weight:600;line-height:1;color:#3fb950">${passRate}%</div>
          <div style="font-size:10px;color:#8b949e;margin-top:4px">${healthyStores}/${totalStores} tests</div>
          <div style="font-size:10px;color:${deltaStyle(passRateDelta)};margin-top:4px">${deltaArrow(passRateDelta)}${Math.abs(passRateDelta)}% vs yesterday</div>
        </div>
        <div style="background:#161b22;border:1px solid #21262d;border-radius:8px;padding:12px 14px">
          <div style="font-size:10px;color:#8b949e;text-transform:uppercase;letter-spacing:.04em;margin-bottom:5px">Missing stores</div>
          <div style="font-size:28px;font-weight:600;line-height:1;color:#d85a30">${missingCount}</div>
          <div style="font-size:10px;color:#8b949e;margin-top:4px">${missingCount} stores</div>
        </div>
        <div style="background:#161b22;border:1px solid #21262d;border-radius:8px;padding:12px 14px">
          <div style="font-size:10px;color:#8b949e;text-transform:uppercase;letter-spacing:.04em;margin-bottom:5px">Flake rate</div>
          <div style="font-size:28px;font-weight:600;line-height:1;color:#d29922">${flakeRate}%</div>
          <div style="font-size:10px;color:#8b949e;margin-top:4px">Based on 14d history</div>
        </div>
        <div style="background:#161b22;border:1px solid #21262d;border-radius:8px;padding:12px 14px">
          <div style="font-size:10px;color:#8b949e;text-transform:uppercase;letter-spacing:.04em;margin-bottom:5px">Avg exec time</div>
          <div style="font-size:20px;font-weight:600;line-height:1;color:#8b949e">—</div>
          <div style="font-size:10px;color:#8b949e;margin-top:4px">Not tracked yet</div>
        </div>
      </div>

      <div style="display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:8px;margin-bottom:14px">
        <div style="background:#161b22;border:1px solid #21262d;border-radius:8px;padding:12px 14px">
          <div style="font-size:10px;color:#8b949e;text-transform:uppercase;letter-spacing:.04em;margin-bottom:5px">Monthly CI cost</div>
          <div style="font-size:20px;font-weight:600;line-height:1;color:#8b949e">—</div>
          <div style="font-size:10px;color:#8b949e;margin-top:4px">Not tracked yet</div>
        </div>
        <div style="background:#161b22;border:1px solid #21262d;border-radius:8px;padding:12px 14px">
          <div style="font-size:10px;color:#8b949e;text-transform:uppercase;letter-spacing:.04em;margin-bottom:5px">Healthy clients</div>
          <div style="font-size:28px;font-weight:600;line-height:1">${healthyStores}/${totalStores}</div>
          <div style="font-size:10px;color:#8b949e;margin-top:4px">${totalStores > 0 ? Math.round((healthyStores/totalStores)*100) : 0}% of total</div>
        </div>
        <div style="background:#161b22;border:1px solid #21262d;border-radius:8px;padding:12px 14px">
          <div style="font-size:10px;color:#8b949e;text-transform:uppercase;letter-spacing:.04em;margin-bottom:5px">Bot block rate</div>
          <div style="font-size:28px;font-weight:600;line-height:1;color:#d29922">${botBlockRate}%</div>
          <div style="font-size:10px;color:#8b949e;margin-top:4px">${botBlocked} of ${totalStores} stores</div>
        </div>
        <div style="background:#161b22;border:1px solid #21262d;border-radius:8px;padding:12px 14px">
          <div style="font-size:10px;color:#8b949e;text-transform:uppercase;letter-spacing:.04em;margin-bottom:5px">MTTR</div>
          <div style="font-size:20px;font-weight:600;line-height:1;color:#8b949e">—</div>
          <div style="font-size:10px;color:#8b949e;margin-top:4px">Not tracked yet</div>
        </div>
        <div style="background:#161b22;border:1px solid #21262d;border-radius:8px;padding:12px 14px">
          <div style="font-size:10px;color:#8b949e;text-transform:uppercase;letter-spacing:.04em;margin-bottom:5px">Deploy corr.</div>
          <div style="font-size:20px;font-weight:600;line-height:1;color:#8b949e">—</div>
          <div style="font-size:10px;color:#8b949e;margin-top:4px">Not tracked yet</div>
        </div>
      </div>

      <div style="background:#161b22;border:1px solid #21262d;border-radius:8px;padding:14px;margin-bottom:12px">
        <div style="font-size:11px;font-weight:500;color:#8b949e;text-transform:uppercase;letter-spacing:.05em;margin-bottom:10px">Store health matrix</div>
        <div style="display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:6px;margin-bottom:10px">
          <div style="padding:7px 10px;border-radius:6px;font-size:10px;font-weight:500;background:#1a3a2a;color:#3fb950">
            banana_republic
            <div style="display:flex;align-items:flex-end;gap:1px;height:12px;margin-top:4px">
              <span style="flex:1;border-radius:1px 1px 0 0;height:80%;background:#3fb950"></span>
              <span style="flex:1;border-radius:1px 1px 0 0;height:85%;background:#3fb950"></span>
              <span style="flex:1;border-radius:1px 1px 0 0;height:88%;background:#3fb950"></span>
              <span style="flex:1;border-radius:1px 1px 0 0;height:90%;background:#3fb950"></span>
              <span style="flex:1;border-radius:1px 1px 0 0;height:92%;background:#3fb950"></span>
              <span style="flex:1;border-radius:1px 1px 0 0;height:95%;background:#3fb950"></span>
              <span style="flex:1;border-radius:1px 1px 0 0;height:100%;background:#3fb950"></span>
            </div>
          </div>
          <div style="padding:7px 10px;border-radius:6px;font-size:10px;font-weight:500;background:#1a3a2a;color:#3fb950">
            gap_japan
            <div style="display:flex;align-items:flex-end;gap:1px;height:12px;margin-top:4px">
              <span style="flex:1;border-radius:1px 1px 0 0;height:78%;background:#3fb950"></span>
              <span style="flex:1;border-radius:1px 1px 0 0;height:80%;background:#3fb950"></span>
              <span style="flex:1;border-radius:1px 1px 0 0;height:82%;background:#3fb950"></span>
              <span style="flex:1;border-radius:1px 1px 0 0;height:85%;background:#3fb950"></span>
              <span style="flex:1;border-radius:1px 1px 0 0;height:88%;background:#3fb950"></span>
              <span style="flex:1;border-radius:1px 1px 0 0;height:90%;background:#3fb950"></span>
              <span style="flex:1;border-radius:1px 1px 0 0;height:90%;background:#3fb950"></span>
          ...
          <div style="padding:7px 10px;border-radius:6px;font-size:10px;font-weight:500;background:#1a3a2a;color:#3fb950">
            ...
            <div style="display:flex;align-items:flex-end;gap:1px;height:12px;margin-top:4px">
              ...
              <span style="flex:1;border-radius:1px 1px 0 0;height:92%;background:#3fb950"></span>
              <span style="flex:1;border-radius:1px 1px 0 0;height:90%;background:#3fb950"></span>
              <span style="flex:1;border-radius:1px 1px 0 0;height:93%;background:#3fb950"></span>
              <span style="flex:1;border-radius:1px 1px 0 0;height:95%;background:#3fb950"></span>
              <span style="flex:1;border-radius:1px 1px 0 0;height:95%;background:#3fb950"></span>
            </div>
          </div>
          ...
          <div style="padding:7px 10px;border-radius:6px;font-size:10px;font-weight:500;background:#3a2a1a;color:#d29922">
            adidas · bot
            <div style="display:flex;align-items:flex-end;gap:1px;height:12px;margin-top:4px">
              <span style="flex:1;border-radius:1px 1px 0 0;height:5%;background:#d29922"></span>
              <span style="flex:1;border-radius:1px 1px 0 0;height:5%;background:#d29922"></span>
              <span style="flex:1;border-radius:1px 1px 0 0;height:5%;background:#d29922"></span>
              <span style="flex:1;border-radius:1px 1px 0 0;height:5%;background:#d29922"></span>
              <span style="flex:1;border-radius:1px 1px 0 0;height:5%;background:#d29922"></span>
          ...
          <div style="padding:7px 10px;border-radius:6px;font-size:10px;font-weight:500;background:#3a1f1a;color:#d85a30">
            snidel · 17 runs
            <div style="display:flex;align-items:flex-end;gap:1px;height:12px;margin-top:4px">
              <span style="flex:1;border-radius:1px 1px 0 0;height:60%;background:#d85a30"></span>
              <span style="flex:1;border-radius:1px 1px 0 0;height:40%;background:#d85a30"></span>
              <span style="flex:1;border-radius:1px 1px 0 0;height:20%;background:#d85a30"></span>
              <span style="flex:1;border-radius:1px 1px 0 0;height:10%;background:#d85a30"></span>
              <span style="flex:1;border-radius:1px 1px 0 0;height:5%;background:#d85a30"></span>
              <span style="flex:1;border-radius:1px 1px 0 0;height:5%;background:#d85a30"></span>
              <span style="flex:1;border-radius:1px 1px 0 0;height:5%;background:#d85a30"></span>
            </div>
          </div>
        ...
        <div style="display:flex;gap:14px;font-size:10px;color:#8b949e">
          <span><span style="display:inline-block;width:8px;height:8px;border-radius:50%;background:#3fb950;margin-right:4px"></span>Healthy (4)</span>
          <span><span style="display:inline-block;width:8px;height:8px;border-radius:50%;background:#d29922;margin-right:4px"></span>Bot blocked (3)</span>
          <span><span style="display:inline-block;width:8px;height:8px;border-radius:50%;background:#d85a30;margin-right:4px"></span>Missing (1)</span>
        </div>
      </div>

      <div style="background:#161b22;border:1px solid #21262d;border-radius:8px;padding:14px;margin-bottom:12px">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px">
          <div style="font-size:11px;font-weight:500;color:#8b949e;text-transform:uppercase;letter-spacing:.05em">Flaky stores</div>
          <div style="display:inline-flex;align-items:center;gap:4px;padding:3px 8px;border-radius:10px;font-size:10px;font-weight:500;background:#3a2a1a;color:#d29922">${flakeRate}% overall</div>
        </div>
        <table style="width:100%;border-collapse:collapse;font-size:11px">
          <thead>
            <tr>
              <th style="padding:7px 8px;border-bottom:1px solid #21262d;font-weight:500;color:#8b949e;font-size:10px;text-align:left;text-transform:uppercase;letter-spacing:.04em">Store</th>
              <th style="padding:7px 8px;border-bottom:1px solid #21262d;font-weight:500;color:#8b949e;font-size:10px;text-align:left;text-transform:uppercase;letter-spacing:.04em">Flake rate</th>
              <th style="padding:7px 8px;border-bottom:1px solid #21262d;font-weight:500;color:#8b949e;font-size:10px;text-align:left;text-transform:uppercase;letter-spacing:.04em">Last flip</th>
              <th style="padding:7px 8px;border-bottom:1px solid #21262d;font-weight:500;color:#8b949e;font-size:10px;text-align:left;text-transform:uppercase;letter-spacing:.04em">Status</th>
            </tr>
          </thead>
          <tbody>
            <tr>
              <td style="padding:8px;border-bottom:1px solid #21262d;font-weight:500">hankyu_hanshin</td>
              <td style="padding:8px;border-bottom:1px solid #21262d">
                <div style="font-size:11px;margin-bottom:2px">57.1%</div>
                <div style="height:4px;border-radius:2px;margin-top:3px;width:57%;background:#f85149"></div>
              </td>
              <td style="padding:8px;border-bottom:1px solid #21262d;color:#8b949e;font-size:10px">Miss→Pass · 5 runs</td>
              <td style="padding:8px;border-bottom:1px solid #21262d">
                <span style="display:inline-flex;align-items:center;gap:4px;padding:3px 8px;border-radius:10px;font-size:10px;font-weight:500;background:#3a2a1a;color:#d29922">Watch</span>
              </td>
            </tr>
            <tr>
              <td style="padding:8px;font-weight:500">lumine</td>
              <td style="padding:8px">
                <div style="font-size:11px;margin-bottom:2px">28.6%</div>
                <div style="height:4px;border-radius:2px;margin-top:3px;width:29%;background:#f85149"></div>
              </td>
              <td style="padding:8px;color:#8b949e;font-size:10px">Pass→Miss · 3 runs</td>
              <td style="padding:8px">
                <span style="display:inline-flex;align-items:center;gap:4px;padding:3px 8px;border-radius:10px;font-size:10px;font-weight:500;background:#3a2a1a;color:#d29922">Watch</span>
              </td>
            </tr>
    ...
    <div id="health-client-panel" class="health-panel" style="display:none">Client dashboard content goes here</div>
    <div id="health-sales-panel" class="health-panel" style="display:none">Sales dashboard content goes here</div>
  </div>
...
function switchHealthTab(tab, btn) {
  ...
  btn.style.borderBottom = '2px solid #f0f6fc';

  document.querySelectorAll('.health-panel').forEach(p => p.style.display = 'none');
  document.getElementById('health-' + tab + '-panel').style.display = 'block';
}
</script>
</body>
</html>`;
