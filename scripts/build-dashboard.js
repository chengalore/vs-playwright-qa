...
function computeMetrics(history) {
  ...
  return {
    ...
    p95WidgetVisibleMs: s.p95WidgetVisibleMs ?? null,
  };
}
...
<main id="content">
  ...
  <div class="panel active" id="panel-monitor">
    ...
    <div class="panel-body">
      ...
      <div style="background:#161b22;border:1px solid #21262d;border-radius:8px;padding:16px 18px;margin-bottom:20px">
        <div style="font-size:11px;font-weight:600;color:#8b949e;text-transform:uppercase;letter-spacing:.06em;margin-bottom:12px">Widget Load Time — Avg vs P95 (14-Day)</div>
        <div id="chart-widgettime" style="height:220px"></div>
      </div>
  ...
  <div class="panel" id="panel-single">
    ...
    <div id="single-latest-screenshot"></div>
    </div>
    </div>
...
function showSingleSubtab(name, btn) {
  ...
  if (name === 'run') renderLatestScreenshot();
}

// ── Latest widget screenshot (Run test subtab) ────────────────────────────────
function renderLatestScreenshot() {
  const el = document.getElementById('single-latest-screenshot');
  if (!el) return;
  const entry = SINGLE_URL_HISTORY[0];
  ...
  const makeScreenshotCard = (src, label, badge, detail) => \`
    <div style="flex:1;min-width:220px;max-width:480px;background:#0d1117;border:1px solid #30363d;border-radius:8px;overflow:hidden">
      <div style="background:#161b22;padding:6px 12px;display:flex;align-items:center;gap:8px;border-bottom:1px solid #30363d">
        ...
        <div style="font-size:10px;color:#8b949e;font-weight:600;text-transform:uppercase;letter-spacing:.05em">\${label}</div>
      </div>
      <img src="\${src}" alt="\${label}" style="width:100%;display:block;max-height:280px;object-fit:cover;object-position:top">
      <div style="padding:6px 12px;display:flex;align-items:center;justify-content:space-between;gap:8px">
        <span style="display:inline-flex;align-items:center;gap:5px;font-size:11px;font-weight:600;color:#3fb950;background:rgba(46,160,67,0.1);border:1px solid rgba(46,160,67,0.3);border-radius:12px;padding:2px 8px"><span style="font-size:8px">●</span> \${badge}</span>
        \${detail ? \`<span style="font-size:11px;color:#8b949e">\${detail}</span>\` : ''}
      </div>
    </div>\`;
  ...
  const screenshotCard = b.onboardingFile
    ? makeScreenshotCard(b.screenshotFile, 'Widget button', 'Widget detected', ms ? \`\${ms}ms\` : null)
      ...
      + (b.onboardingFilledFile ? makeScreenshotCard(b.onboardingFilledFile, 'Onboarding filled', 'Before submit', null) : '')
  ...
  if (ms && b.hasSmartTable === true && b.widgetType && ['inpage','inpage_luxury','inpage_mini'].includes(b.widgetType)) {
    checks.push({ label: 'Smart table present', val: 'yes', pass: true });
  }
...
function showPanel(name) {
  ...
  document.getElementById('btn-' + name).classList.add('active');
  if (name === 'compare') renderCompareView();
  if (name === 'single') { renderSingleUrl(); renderLatestScreenshot(); }
  if (name === 'cost') renderCostPanel();
}
...
function renderSingleDetail(entry, idx) {
  ...
  const screenshotCardsHtml = screenshotBrowsers.flatMap(b => {
    const ms = b.widgetVisibleMs;
    const mkCard = (src, label, badge, detail) =>
      \`<div style="background:#0d1117;border:1px solid #30363d;border-radius:8px;overflow:hidden;max-width:480px;flex:1;min-width:220px">
        <div style="background:#161b22;padding:6px 12px;display:flex;align-items:center;gap:8px;border-bottom:1px solid #30363d">
          <div style="display:flex;gap:4px;flex-shrink:0">
            <div style="width:9px;height:9px;border-radius:50%;background:#ff5f57"></div>
            <div style="width:9px;height:9px;border-radius:50%;background:#febc2e"></div>
          ...
          <div style="font-size:10px;color:#8b949e;font-weight:600;text-transform:uppercase;letter-spacing:.05em">\${label}</div>
          <span style="font-size:10px;color:#484f58;margin-left:auto">\${b.browser}</span>
        </div>
        <img src="\${src}" alt="\${label}" style="width:100%;display:block;max-height:260px;object-fit:cover;object-position:top">
        <div style="padding:6px 12px;display:flex;align-items:center;justify-content:space-between;gap:8px">
          <span style="display:inline-flex;align-items:center;gap:5px;font-size:11px;font-weight:600;color:#3fb950;background:rgba(46,160,67,0.1);border:1px solid rgba(46,160,67,0.3);border-radius:12px;padding:2px 8px"><span style="font-size:8px">●</span> \${badge}</span>
          \${detail ? \`<span style="font-size:11px;color:#8b949e">\${detail}</span>\` : ''}
        </div>
    ...
    if (b.onboardingFilledFile) cards.push(mkCard(b.onboardingFilledFile, 'Onboarding filled', 'Before submit', null));
  ...
  \${(() => {
    const TEXT_SNAPSHOTS = [
      { key: 'widget-button',      label: 'Widget button' },
      { key: 'onboarding',         label: 'Onboarding open' },
      { key: 'onboarding-filled',  label: 'Onboarding filled' },
      { key: 'result',             label: 'Size result' },
      { key: 'gift-onboarding',    label: 'Gift onboarding' },
      { key: 'gift-result',        label: 'Gift result' },
    ];
    const textBrowsers = (entry.browsers || []).filter(b => b.widgetTextLog);
    if (!textBrowsers.length) return '';
    const rows = textBrowsers.map(b => {
      const snaps = TEXT_SNAPSHOTS.map(({ key, label }) => {
        const texts = b.widgetTextLog?.[key];
        if (!texts?.length) return '';
        return \`<div style="margin-bottom:8px">
          <div style="font-size:10px;color:#484f58;text-transform:uppercase;letter-spacing:0.5px;margin-bottom:3px">\${label}</div>
          <div style="font-size:12px;color:#c9d1d9;line-height:1.6;word-break:break-word">\${texts.map(t => \`<span style="display:inline-block;background:#161b22;border:1px solid #30363d;border-radius:4px;padding:1px 6px;margin:1px 3px 1px 0;font-size:11px">\${t.replace(/</g,'&lt;').replace(/>/g,'&gt;')}</span>\`).join('')}</div>
        </div>\`;
      }).join('');
      if (!snaps) return '';
      return \`<div style="font-size:10px;color:#8b949e;margin-bottom:6px;text-transform:uppercase;letter-spacing:0.5px">\${b.browser}</div>\${snaps}\`;
    }).join('');
    if (!rows) return '';
    return \`<details style="margin-top:16px">
      <summary style="cursor:pointer;font-size:11px;text-transform:uppercase;letter-spacing:0.6px;color:#484f58;font-weight:600;user-select:none">Widget Text Log</summary>
    ...
    const diffRows = bs.flatMap(b => {
      ...
      return Object.entries(b.textDiff).map(([screen, {missing, added}]) => {
        ...
        return \`<div style="margin-bottom:10px">
          <div style="font-size:10px;color:#484f58;text-transform:uppercase;letter-spacing:0.5px;margin-bottom:4px">\${snapLabel} — \${b.browser}</div>
          \${missing.map(t => \`<div style="font-size:12px;color:#f85149;padding:2px 0;display:flex;gap:6px;align-items:baseline"><span style="font-size:10px;font-weight:700;flex-shrink:0">−</span><span>\${t.replace(/</g,'&lt;').replace(/>/g,'&gt;')}</span></div>\`).join('')}
          \${added.map(t => \`<div style="font-size:12px;color:#e3b341;padding:2px 0;display:flex;gap:6px;align-items:baseline"><span style="font-size:10px;font-weight:700;flex-shrink:0">+</span><span>\${t.replace(/</g,'&lt;').replace(/>/g,'&gt;')}</span></div>\`).join('')}
        </div>\`;
      });
    }).join('');
    return \`<div style="margin-top:16px;padding-top:14px;border-top:1px solid #21262d">
...
async function triggerSingleRun() {
  ...
  const setBaseline    = document.getElementById('single-set-baseline').checked;
  ...
  const res = await fetch('https://api.github.com/repos/chengalore/vs-playwright-qa/actions/workflows/single-url-test.yml/dispatches', {
    ...
    body: JSON.stringify({ ref: 'main', inputs: {
      ...
      set_text_baseline: setBaseline ? 'true' : 'false',
    }}),
  });
...
function showComparison(entryIdx) {
  ...
  const rows = browsers.map(browser => {
    ...
    const onboardingRow = (eb?.onboardingFile || bb?.onboardingFile)
      ...
      : '';
    const onboardingFilledRow = (eb?.onboardingFilledFile || bb?.onboardingFilledFile)
      ? \`<div style="margin-bottom:8px">
          <div style="font-size:10px;color:#484f58;text-transform:uppercase;margin-bottom:6px">Onboarding filled</div>
          <div style="display:flex;gap:12px">
            \${eb?.onboardingFilledFile ? mkCard(eb.onboardingFilledFile, entryLabel)    : \`<div style="flex:1;min-width:0;color:#484f58;font-size:12px">No screenshot</div>\`}
            \${bb?.onboardingFilledFile ? mkCard(bb.onboardingFilledFile, baselineLabel) : \`<div style="flex:1;min-width:0;color:#484f58;font-size:12px">No screenshot</div>\`}
          </div>
        </div>\`
      : '';
    ...
    const smartTableRow = (eb?.smartTableFile || bb?.smartTableFile)
      ...
      : '';
    return widgetRow || onboardingRow || onboardingFilledRow || resultRow || giftOnboardingRow || giftResultRow || smartTableRow
      ? \`<div style="background:#0d1117;border:1px solid #21262d;border-radius:8px;padding:14px;margin-bottom:10px">
          ...
          \${widgetRow}\${onboardingRow}\${onboardingFilledRow}\${resultRow}\${giftOnboardingRow}\${giftResultRow}\${smartTableRow}
        </div>\`
      : '';
  }).filter(Boolean).join('');
...
function renderCostPanel() {
  ...
  const storeData = STORE_LIST.filter(store => !botSet.has(store)).map(store => {
    let color, dur;
    if (newMissSet.has(store)) { color = '#f85149'; dur = COST_DUR.missing; }
...
function makeDualLineSvgChart(vals1, vals2, labels, color1, color2, unit, dec) {
  const allVals = [...vals1, ...vals2].filter(v => v != null);
  if (allVals.length < 2) return \`<p style="color:#8b949e;font-size:12px;padding:20px">Not enough data</p>\`;
  const W = 900, H = 220, ml = 48, mr = 20, mt = 36, mb = 32;
  const cw = W - ml - mr, ch = H - mt - mb;
  const minV = Math.min(...allVals), maxV = Math.max(...allVals);
  const spread = maxV - minV || 1;
  const yMin = Math.max(0, minV - spread * 0.18);
  const yMax = maxV + spread * 0.18;
  const yRange = yMax - yMin || 1;
  const n = vals1.length;
  const xS = i => ml + (i / Math.max(n - 1, 1)) * cw;
  const yS = v => mt + ch - ((v - yMin) / yRange * ch);
  const steps = 4;
  let grid = '', xlbls = '';
  for (let i = 0; i <= steps; i++) {
  ...
  const pts1 = vals1.map((v, i) => \`\${xS(i).toFixed(1)},\${yS(v).toFixed(1)}\`).join(' ');
  const dots1 = vals1.map((v, i) => \`<circle cx="\${xS(i).toFixed(1)}" cy="\${yS(v).toFixed(1)}" r="3" fill="\${color1}" stroke="#161b22" stroke-width="1.5"/>\`).join('');
  const pts2 = vals2.map((v, i) => \`\${xS(i).toFixed(1)},\${yS(v).toFixed(1)}\`).join(' ');
  const dots2 = vals2.map((v, i) => \`<circle cx="\${xS(i).toFixed(1)}" cy="\${yS(v).toFixed(1)}" r="3" fill="\${color2}" stroke="#161b22" stroke-width="1.5"/>\`).join('');
  const lx = (W / 2) - 70;
  const legend = \`<rect x="\${lx}" y="8" width="12" height="12" fill="none" stroke="\${color1}" stroke-width="2"/><text x="\${lx+16}" y="18" fill="#8b949e" font-size="10">avg</text><rect x="\${lx+55}" y="8" width="12" height="12" fill="none" stroke="\${color2}" stroke-width="2" stroke-dasharray="3,2"/><text x="\${lx+71}" y="18" fill="#8b949e" font-size="10">p95</text>\`;
  return \`<svg viewBox="0 0 \${W} \${H}" xmlns="http://www.w3.org/2000/svg" style="width:100%;height:100%">
    \${grid}
    <polyline points="\${pts1}" fill="none" stroke="\${color1}" stroke-width="2" stroke-linejoin="round" stroke-linecap="round"/>
    \${dots1}
    <polyline points="\${pts2}" fill="none" stroke="\${color2}" stroke-width="2" stroke-linejoin="round" stroke-linecap="round" stroke-dasharray="6,3"/>
    \${dots2}
    \${xlbls}\${legend}
  </svg>\`;
}
...
function renderTrendCharts() {
  ...
  const timedRuns = runs.filter(r => typeof r.summary?.avgWidgetVisibleMs === 'number');
  if (timedRuns.length >= 2) {
    const avgS  = timedRuns.map(r => Math.round(r.summary.avgWidgetVisibleMs / 100) / 10);
    const p95S  = timedRuns.map(r => typeof r.summary?.p95WidgetVisibleMs === 'number'
      ? Math.round(r.summary.p95WidgetVisibleMs / 100) / 10
      : Math.round(r.summary.avgWidgetVisibleMs / 100) / 10);
    const wDates = timedRuns.map(r => new Date(r.timestamp).toLocaleDateString('en-GB', { day: 'numeric', month: 'short' }));
    document.getElementById('chart-widgettime').innerHTML = makeDualLineSvgChart(avgS, p95S, wDates, '#58a6ff', '#e09432', 's', 1);
  }
}
...
function detectLangFromUrl(url) {
  if (!url) return 'en';
  const h = url.split('?')[0].split('#')[0];
  if (h.includes('.co.jp')) return 'ja';
  if (h.includes('.co.kr')) return 'ko';
  if (h.includes('.com.tw') || h.includes('.com.hk')) return 'zh-tw';
  if (h.includes('.cn') && !h.includes('cdn')) return 'zh-cn';
  return 'en';
}
const LANG_LABELS = { en: 'English', ja: '日本語', ko: '한국어', 'zh-tw': '繁體中文', 'zh-cn': '简体中文' };
