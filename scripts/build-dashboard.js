    ...
    <div style="background:#161b22;border:1px solid #21262d;border-radius:8px;padding:16px;margin-bottom:24px">
      ...
      <div style="margin-bottom:10px">
        ...
        <input id="gh-pat" type="password" placeholder="ghp_xxxxxxxxxxxx"
          ...
          oninput="localStorage.setItem('gh_pat', this.value)">
      </div>
      <div style="margin-bottom:10px">
...
function renderSingleUrl() {
  ...
  if (SINGLE_URL_HISTORY.length === 0) {
    tbody.innerHTML = '<tr><td colspan="9" class="empty-state">No single URL tests yet.</td></tr>';
    return;
  }
  ...
  tbody.innerHTML = SINGLE_URL_HISTORY.map((entry, idx) => {
    ...
    return \`<tr id="single-row-\${idx}" style="cursor:pointer" onclick="toggleSingleFlag(\${idx}, \${JSON.stringify(entry.url || '')})">
